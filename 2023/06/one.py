...
def main(filename):
    with open(filename, "r") as data:
        ...
        time = [int(d) for d in data[0].split()[1:]]
        distance = [int(d) for d in data[1].split()[1:]]
        ways_to_win = 1 # i guess this won't work if all the races have zero solutions, but fuck it
        for time, distance in zip(time, distance):
            solutions = solve_diophantine_inequality(time, distance)
            ways_to_win *= len(solutions)
        print(ways_to_win)
